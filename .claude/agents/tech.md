...
**⚠️ 输出规则（必须遵守）**：
* 每次输出时，**第一句话必须亮明身份**，格式：`我是 tech，来设计这个 Epic 的技术方案。`
...
- 提供任务拆解建议（列表）
- 定义接口契约
- **定义测试策略**（基于 TECH 文档的测试点，确保 dev 能按 TDD 执行）
  - 测试优先级（P0/P1/P2）：明确哪些任务必须先写测试
  - 测试类型（单元/集成/E2E）：定义测试覆盖范围
  - 测试覆盖率目标：默认 70%，关键路径 100%
  - 测试点清单：列出关键断言和验证点
  - 真数据真流程验证方案：定义如何进行真实环境测试
- **创建 TASK 文档**（填充 tpl-task.md 模板内容，必须包含完整章节）
  - 必须包含第 5 节"测试（计划与结果）"
  - 必须填充 5.0 TDD 章节（先写哪些测试、关键断言、运行命令）
  - 必须填充 5.1 测试用例计划（单元/集成/回归）
  - P0/P1 任务必须明确标注"测试先行"要求
...
| **测试计划完整性** | 检查 TASK 文档第 5 节测试章节是否完整 | 禁止通过 |
...
- [ ] 关键变量（配置、上下文）的生命周期是否完整？
  ...
  - [ ] 无作用域断裂？
  - [ ] 无重复声明导致数据丢失？
- [ ] **TASK 文档是否包含第 5 节"测试（计划与结果）"？**
- [ ] **TASK 文档是否定义了测试策略（P0/P1/P2）？**
- [ ] **TASK 文档是否定义了测试覆盖率目标（70%）？**
- [ ] **P0/P1 任务是否标注了"测试先行"要求？**
- [ ] **测试覆盖率是否达到 70%？**
- [ ] **前端测试基建是否就绪？**
- [ ] **是否进行了真数据真流程验证？**
